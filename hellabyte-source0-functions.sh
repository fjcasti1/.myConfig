...
bakup() {
  ...
  waitTime="${2:-10s}"
  NDir=$(find $dir -type f |wc -l)
  bakupdir="bakup${dir}"
  echo "Directory: ${dir}"
  echo "Number of Files: ${NDir}"
  echo "Creating Bakup Directory: ${bakupdir}"
  cp -r $dir $bakupdir &
  NbakupDir=$(find $bakupdir -type f |wc -l)
  while [ "$NbakupDir" -lt "$NDir" ];
  do
    sleep ${waitTime}
    NbakupDir=$(find $bakupdir -type f |wc -l)
    python <<__EOF
...
N      = float(${NbakupDir})
NTotal = float(${NDir})
print(str(round(N*100/NTotal,1))+' %')
__EOF
  done
}

hmf() {
  pattern="${1:?'Pattern NOT Provided'}"
  echo "$(ls ${pattern}* | wc -l)"
}
...
gitbc(){
  name=${1:?'BRANCH NAME NOT PROVIDED'}
  git checkout -b $name
